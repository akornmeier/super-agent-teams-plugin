...
3. **Override `team_pattern` resolution.** When an override forces a different skill (e.g., `stack_trace_present` → `/debug`), the resolved `team_pattern` MUST match the FORCED skill's native pattern, not the originally matched rule's. Look up the forced skill's `team_pattern` from the rule whose `skill:` field equals the forced skill. If no such rule exists, escalate as a routing bug. (Note: `tests/test_routing.py` currently asserts the originally matched rule's `team_pattern` survives override — that assertion needs to be extended in Task 18; track as a follow-up.)
